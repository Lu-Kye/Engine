...
void InitGraphics() {
    // 坐标系的原点在视口的中心，因为我们只有一个视口，所以就是屏幕中心。
    // 视口在各个坐标轴的缺省的范围是[-1, 1]，因此0.5差不多正好是在画布长宽各1/4的地方。
    // 第二个部分是颜色。分别对应R（红色通道）G（绿色通道）B（蓝色通道）和A（透明通道）。
    // 这个顺序是在代码的layout部分指定的。范围是[0, 1]。参考 struct VERTEX 的结构
